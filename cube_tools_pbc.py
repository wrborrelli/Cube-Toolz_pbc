...
def mipos(pos_vec, bl):
    return np.array(pos_vec).astype('float32') - bl*np.round(np.array(pos_vec).astype('float32')/bl)
...
class cube():
    ...
    def cube_int_atom(self, atomID, radius, bl=0.0, pbc=False):
        '''
        ...
        bl *= 1 / (physical_constants['Bohr radius'][0] * 1e10)
        ...
        dv = ZYX - atomXYZ
        if pbc == 'True':
            print('Doing PBC!',flush=True)
            dv = [mipos(dv[i].reshape((dv[i].shape[0]*dv[i].shape[1]*dv[i].shape[2])),bl).reshape((dv[i].shape[0],dv[i].shape[1],dv[i].shape[2])) for i in range(len(dv))]
            dist_from_center = np.linalg.norm(dv)
            print([np.min(dist_from_center),np.max(dist_from_center)],flush=True)
        else:
            print('No PBC!',flush=True)
            dv = ZYX - atomXYZ
            dist_from_center = np.linalg.norm(dv)
            print([np.min(dist_from_center),np.max(dist_from_center)],flush=True)
    ...
    def cube_int_ref(self,ref,radius, bl=0.0, pbc=False):
        '''
...
def cube_integrate_atom(files, atomId, radius, bl, pbc):
    cube_in = cube(files[0])
    atomId = int(atomId)
    radius = float(radius)
    bl = float(bl)
    pbc = pbc
    cube_int_total = cube_in.cube_int_atom(atomId, radius, bl, pbc)
    print( 'Integral around chosen atom is: %12.9f' % cube_int_total)
    return None

def cube_integrate_ref(files, x, y, z, radius, bl, pbc):
    cube_in = cube(files[0])
    xyz = np.array([float(x), float(y), float(z)])
    radius = float(radius)
    bl = float(bl)
    pbc = pbc
    cube_int_total = cube_in.cube_int_ref(xyz, radius, bl, pbc)
...
#    Take the planar average")
    ...
    parser.add_argument("-ia","--integrateatom",help="Integrate a sphere around a particular atom. Needs atom id and a radius (in Angstrom) to integrate within.", nargs=4 )
    parser.add_argument("-ir","--integrateref",help="Integrate a sphere around a reference xyz (given in Angstrom). Supply x y z r, where r is the radius (in Angstrom).", nargs=6 )
    ...
    if args.integrateatom:
        if args.Files:
            cube_integrate_atom(args.Files, args.integrateatom[0], args.integrateatom[1], args.integrateatom[2], args.integrateatom[3])
    if args.integrateref:
        if args.Files:
            cube_integrate_ref(args.Files, args.integrateref[0], args.integrateref[1], args.integrateref[2], args.integrateref[3], args.integrateatom[4], args.integrateatom[5])
    if args.rotate:
        if args.Files:
            if args.axis:
